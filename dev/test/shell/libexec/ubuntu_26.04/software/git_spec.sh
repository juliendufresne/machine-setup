# Specs for the git unit on Ubuntu 26.04. TEST_FLAG keeps the unit's functions
...
Describe 'libexec/ubuntu_26.04/software/git.sh'
    TEST_FLAG=true
    Include libexec/ubuntu_26.04/software/git.sh

    # Isolated host and a log to spy on apt-get. GIT_ON_PATH drives is_installed
    # (does `command -v git` resolve) and GIT_PRESENT drives is_managed (does
    # `dpkg -s git` report the package), so a test can pretend git is present by
    # any means, present via our package, or absent, without touching the host.
    setup() {
        helper::isolate
        export MACHINE_SETUP_UNIT_NAME=git
    ...
    # `command -v git`; every other call falls through to the real builtin.
    command() {
        if [ "$1" = -v ] && [ "$2" = git ]
        then [ "${GIT_ON_PATH}" = 1 ]
    ...
    Describe 'unit::is_installed'
        ...
        It 'is true when git resolves on the host'
            GIT_ON_PATH=1
        ...
        It 'is false when git resolves nowhere'
            GIT_ON_PATH=0
            When call unit::is_installed
    ...
    Describe 'unit::is_managed'
        ...
        It 'is true when dpkg reports the git package present'
            GIT_PRESENT=1
        ...
        It 'is false when dpkg reports the git package absent'
            GIT_PRESENT=0
            When call unit::is_managed
            The status should be failure
            The stdout should be blank
    ...
    Describe 'unit::is_configured'
        ...
        It 'is always satisfied because git needs no configuration'
    ...
    Describe 'unit::install'
        ...
        It 'updates the package lists then installs git and reports each step'
            When call unit::install
            The status should be success
            The line 1 of stdout should equal '  ✓ Updating the package lists'
            The line 2 of stdout should equal '  ✓ Installing the git package'
            The stderr should be blank
...
install -y git'
    ...
    Describe 'runner::status'
        ...
        It 'reports available when git is not present'
            GIT_ON_PATH=0
        ...
        It 'reports unmanaged when git is present but not via our package'
            GIT_ON_PATH=1
            GIT_PRESENT=0
            When call runner::status
            The status should be success
        ...
        It 'reports configured as soon as git is present via our package'
            GIT_ON_PATH=1
            GIT_PRESENT=1
            When call runner::status
            The status should be success
    ...
    Describe 'unit::configure'
        ...
        It 'does nothing because git needs no configuration'
    ...
    Describe 'unit::unconfigure'
        ...
        It 'does nothing because git has no configuration to restore'
    ...
    Describe 'unit::uninstall'
        ...
        It 'purges git and reports the step'
            When call unit::uninstall
            The status should be success
            The stdout should equal '  ✓ Purging the git package'
            The stderr should be blank
            The contents of file "$APT_LOG" should equal 'purge -y git'
        End

        It 'purges git even when this tool never recorded installing it'
