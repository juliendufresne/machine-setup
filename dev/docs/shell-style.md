...
- **Software unit scripts** (`libexec/<os>_<version>/software/<name>.sh`) follow
  ...
  `unit::` prefix (rule 14).
- **Identity helper scripts** (`libexec/identity/*`) are the same kind of
  exception: the several files (`ssh-keygen`, `gpg-keygen`, `host-alias`,
  `auth-probe`, ...) form one identity toolkit and share the `identity::` prefix
  (the directory), not their filename. A filename prefix would also collide with
  libraries that already own a namespace, for example `host-alias` against
  `lib/host.sh`'s `host::`. They are sourced fragments (Functions + Constants
  only, no Main/Execute), sourced by the workspace and dotfiles units, the same
  way a unit's `configure` fragment is sourced.
