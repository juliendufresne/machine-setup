...
#
# Description:
#   Reports whether pass can be installed on this host. pass is a command-line
...
#
# Description:
#   Reports whether pass is present on the host by any means: a packaged install,
...
#
# Returns:
#   0 when pass is present on the host
#   1 when pass is not present
#
# Example:
#   unit::is_installed
#--------------------------------------------------
...
#
# Description:
#   Reports whether pass is installed through the mechanism this unit uses, the
#   apt/dpkg package, by querying dpkg. The runner compares this against
#   unit::is_installed: present but unmanaged means pass arrived by some other route
...
#
# Returns:
#   0 when the pass package is installed
#   1 when the pass package is not installed
#
# Example:
#   unit::is_managed
#--------------------------------------------------
unit::is_managed() {
    dpkg -s pass &>/dev/null
}
...
#
# Description:
#   pass has no inputs to collect. It only warms the sudo session, because the
...
#
# Description:
#   Installs pass, or updates it when already present, with apt-get. Each step is
...
unit::install() {
    ...
    output::run 'Installing the pass package' sudo apt-get install -y pass
}
...
#
# Description:
#   Purges pass with apt-get, announced through output::run so the user sees the
#   step and only sees apt-get's output if it fails. Uninstall is unconditional: it
#   removes pass regardless of whether this tool installed it, so a user who asks
...
#--------------------------------------------------
unit::uninstall() {
    output::run 'Purging the pass package' sudo apt-get purge -y pass
}
...
[[ -v TEST_FLAG ]] || readonly -f pass::main
