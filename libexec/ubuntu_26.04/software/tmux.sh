...
#
# Description:
#   Reports whether tmux can be installed on this host. tmux is a command-line
...
#
# Description:
#   Reports whether tmux is present on the host by any means: a packaged install,
...
#
# Returns:
#   0 when tmux is present on the host
#   1 when tmux is not present
#
# Example:
#   unit::is_installed
#--------------------------------------------------
...
#
# Description:
#   Reports whether tmux is installed through the mechanism this unit uses, the
#   apt/dpkg package, by querying dpkg. The runner compares this against
#   unit::is_installed: present but unmanaged means tmux arrived by some other route
...
#
# Returns:
#   0 when the tmux package is installed
#   1 when the tmux package is not installed
#
# Example:
#   unit::is_managed
#--------------------------------------------------
...
#
# Description:
#   tmux has no inputs to collect. It only warms the sudo session, because the
...
#
# Description:
#   Installs tmux, or updates it when already present, with apt-get. Each step is
...
unit::install() {
    ...
    output::run 'Installing the tmux package' sudo apt-get install -y tmux
}
...
#
# Description:
#   Purges tmux with apt-get, announced through output::run so the user sees the
#   step and only sees apt-get's output if it fails. Uninstall is unconditional: it
#   removes tmux regardless of whether this tool installed it, so a user who asks
...
#--------------------------------------------------
unit::uninstall() {
    output::run 'Purging the tmux package' sudo apt-get purge -y tmux
}
