# Specs for the pass unit on Ubuntu 26.04. TEST_FLAG keeps the unit's functions
...
Describe 'libexec/ubuntu_26.04/software/pass.sh'
    TEST_FLAG=true
    Include libexec/ubuntu_26.04/software/pass.sh

    # Isolated host and a log to spy on apt-get. PASS_ON_PATH drives is_installed
    # (does `command -v pass` resolve) and PASS_PRESENT drives is_managed (does
    # `dpkg -s pass` report the package), so a test can pretend pass is present by
    # any means, present via our package, or absent, without touching the host.
    setup() {
        helper::isolate
        export MACHINE_SETUP_UNIT_NAME=pass
    ...
    # `command -v pass`; every other call falls through to the real builtin.
    command() {
        if [ "$1" = -v ] && [ "$2" = pass ]
        then [ "${PASS_ON_PATH}" = 1 ]
    ...
    dpkg()    { [ "${PASS_PRESENT}" = 1 ]; }            # `dpkg -s pass`
    ...
    Describe 'unit::is_installed'
        ...
        It 'is true when pass resolves on the host'
            PASS_ON_PATH=1
        ...
        It 'is false when pass resolves nowhere'
            PASS_ON_PATH=0
            When call unit::is_installed
    ...
    Describe 'unit::is_managed'
        ...
        It 'is true when dpkg reports the pass package present'
            PASS_PRESENT=1
        ...
        It 'is false when dpkg reports the pass package absent'
            PASS_PRESENT=0
            When call unit::is_managed
            The status should be failure
            The stdout should be blank
    ...
    Describe 'unit::is_configured'
        ...
        It 'is always satisfied because pass needs no configuration'
    ...
    Describe 'unit::install'
        ...
        It 'updates the package lists then installs pass and reports each step'
            When call unit::install
            The status should be success
            The line 1 of stdout should equal '  ✓ Updating the package lists'
            The line 2 of stdout should equal '  ✓ Installing the pass package'
            The stderr should be blank
...
install -y pass'
    ...
    Describe 'runner::status'
        ...
        It 'reports available when pass is not present'
            PASS_ON_PATH=0
        ...
        It 'reports unmanaged when pass is present but not via our package'
            PASS_ON_PATH=1
            PASS_PRESENT=0
            When call runner::status
            The status should be success
        ...
        It 'reports configured as soon as pass is present via our package'
            PASS_ON_PATH=1
            PASS_PRESENT=1
            When call runner::status
            The status should be success
    ...
    Describe 'unit::configure'
        ...
        It 'does nothing because pass needs no configuration'
    ...
    Describe 'unit::unconfigure'
        ...
        It 'does nothing because pass has no configuration to restore'
    ...
    Describe 'unit::uninstall'
        ...
        It 'purges pass and reports the step'
            When call unit::uninstall
            The status should be success
            The stdout should equal '  ✓ Purging the pass package'
            The stderr should be blank
            The contents of file "$APT_LOG" should equal 'purge -y pass'
        End

        It 'purges pass even when this tool never recorded installing it'
