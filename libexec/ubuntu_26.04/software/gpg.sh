...
#
# Description:
#   Reports whether gpg can be installed on this host. gpg is a command-line
...
#
# Description:
#   Reports whether gpg is present on the host by any means: a packaged install,
...
#
# Returns:
#   0 when gpg is present on the host
#   1 when gpg is not present
#
# Example:
#   unit::is_installed
#--------------------------------------------------
...
#
# Description:
#   Reports whether gpg is installed through the mechanism this unit uses, the
#   apt/dpkg package, by querying dpkg. The runner compares this against
#   unit::is_installed: present but unmanaged means gpg arrived by some other route
...
#
# Returns:
#   0 when the gpg package is installed
#   1 when the gpg package is not installed
#
# Example:
#   unit::is_managed
#--------------------------------------------------
...
#
# Description:
#   gpg has no inputs to collect. It only warms the sudo session, because the
...
#
# Description:
#   Installs gpg, or updates it when already present, with apt-get. Each step is
...
unit::install() {
    ...
    output::run 'Installing the gpg package' sudo apt-get install -y gpg
}
...
#
# Description:
#   Purges gpg with apt-get, announced through output::run so the user sees the
#   step and only sees apt-get's output if it fails. Uninstall is unconditional: it
#   removes gpg regardless of whether this tool installed it, so a user who asks
...
#--------------------------------------------------
unit::uninstall() {
    output::run 'Purging the gpg package' sudo apt-get purge -y gpg
}
