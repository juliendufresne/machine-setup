...
#
# Description:
#   Reports whether git can be installed on this host. git is a command-line
...
#
# Description:
#   Reports whether git is present on the host by any means: a packaged install,
...
#
# Returns:
#   0 when git is present on the host
#   1 when git is not present
#
# Example:
#   unit::is_installed
#--------------------------------------------------
...
#
# Description:
#   Reports whether git is installed through the mechanism this unit uses, the
#   apt/dpkg package, by querying dpkg. The runner compares this against
#   unit::is_installed: present but unmanaged means git arrived by some other route
...
#
# Returns:
#   0 when the git package is installed
#   1 when the git package is not installed
#
# Example:
#   unit::is_managed
#--------------------------------------------------
...
#
# Description:
#   git has no inputs to collect. It only warms the sudo session, because the
...
#
# Description:
#   Installs git, or updates it when already present, with apt-get. Each step is
...
#
# Description:
#   Purges git with apt-get, announced through output::run so the user sees the
#   step and only sees apt-get's output if it fails. Uninstall is unconditional: it
#   removes git regardless of whether this tool installed it, so a user who asks
...
#--------------------------------------------------
unit::uninstall() {
    output::run 'Purging the git package' sudo apt-get purge -y git
}
