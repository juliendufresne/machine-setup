...
#
# Description:
#   Reports whether tree can be installed on this host. tree is a command-line
...
#
# Description:
#   Reports whether tree is present on the host by any means: a packaged install,
...
#
# Returns:
#   0 when tree is present on the host
#   1 when tree is not present
#
# Example:
#   unit::is_installed
#--------------------------------------------------
...
#
# Description:
#   Reports whether tree is installed through the mechanism this unit uses, the
#   apt/dpkg package, by querying dpkg. The runner compares this against
#   unit::is_installed: present but unmanaged means tree arrived by some other route
...
#
# Returns:
#   0 when the tree package is installed
#   1 when the tree package is not installed
#
# Example:
#   unit::is_managed
#--------------------------------------------------
...
#
# Description:
#   tree has no inputs to collect. It only warms the sudo session, because the
...
#
# Description:
#   Installs tree, or updates it when already present, with apt-get. Each step is
...
unit::install() {
    ...
    output::run 'Installing the tree package' sudo apt-get install -y tree
}
...
#
# Description:
#   Purges tree with apt-get, announced through output::run so the user sees the
#   step and only sees apt-get's output if it fails. Uninstall is unconditional: it
#   removes tree regardless of whether this tool installed it, so a user who asks
...
#--------------------------------------------------
unit::uninstall() {
    output::run 'Purging the tree package' sudo apt-get purge -y tree
}
...
[[ -v TEST_FLAG ]] || readonly -f tree::main
