...
#
# Description:
#   Reports whether curl can be installed on this host. curl is a command-line
...
#
# Description:
#   Reports whether curl is present on the host by any means: a packaged install,
...
#
# Returns:
#   0 when curl is present on the host
#   1 when curl is not present
#
# Example:
#   unit::is_installed
#--------------------------------------------------
...
#
# Description:
#   Reports whether curl is installed through the mechanism this unit uses, the
#   apt/dpkg package, by querying dpkg. The runner compares this against
#   unit::is_installed: present but unmanaged means curl arrived by some other route
...
#
# Returns:
#   0 when the curl package is installed
#   1 when the curl package is not installed
#
# Example:
#   unit::is_managed
#--------------------------------------------------
...
#
# Description:
#   curl has no inputs to collect. It only warms the sudo session, because the
...
#
# Description:
#   Installs curl, or updates it when already present, with apt-get. Each step is
...
unit::install() {
    ...
    output::run 'Installing the curl package' sudo apt-get install -y curl
}
...
#
# Description:
#   Purges curl with apt-get, announced through output::run so the user sees the
#   step and only sees apt-get's output if it fails. Uninstall is unconditional: it
#   removes curl regardless of whether this tool installed it, so a user who asks
...
#--------------------------------------------------
unit::uninstall() {
    output::run 'Purging the curl package' sudo apt-get purge -y curl
}
...
[[ -v TEST_FLAG ]] || readonly -f curl::main
