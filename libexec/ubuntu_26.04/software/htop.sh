...
#
# Description:
#   Reports whether htop can be installed on this host. htop is a command-line
...
#
# Description:
#   Reports whether htop is present on the host by any means: a packaged install,
...
#
# Returns:
#   0 when htop is present on the host
#   1 when htop is not present
#
# Example:
#   unit::is_installed
#--------------------------------------------------
...
#
# Description:
#   Reports whether htop is installed through the mechanism this unit uses, the
#   apt/dpkg package, by querying dpkg. The runner compares this against
#   unit::is_installed: present but unmanaged means htop arrived by some other route
...
#
# Returns:
#   0 when the htop package is installed
#   1 when the htop package is not installed
#
# Example:
#   unit::is_managed
#--------------------------------------------------
...
#
# Description:
#   htop has no inputs to collect. It only warms the sudo session, because the
...
#
# Description:
#   Installs htop, or updates it when already present, with apt-get. Each step is
...
unit::install() {
    ...
    output::run 'Installing the htop package' sudo apt-get install -y htop
}
...
#
# Description:
#   Purges htop with apt-get, announced through output::run so the user sees the
#   step and only sees apt-get's output if it fails. Uninstall is unconditional: it
#   removes htop regardless of whether this tool installed it, so a user who asks
...
#--------------------------------------------------
unit::uninstall() {
    output::run 'Purging the htop package' sudo apt-get purge -y htop
}
...
[[ -v TEST_FLAG ]] || readonly -f htop::main
