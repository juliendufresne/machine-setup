...
# lib/software/fish/configure fragment, sourced below; everything else is here.
...
#
# Description:
#   Reports whether fish can be installed on this host. fish is a command-line
...
#
# Description:
#   Reports whether fish is present on the host by any means: a packaged install,
...
#
# Returns:
#   0 when fish is present on the host
#   1 when fish is not present
#
# Example:
#   unit::is_installed
#--------------------------------------------------
...
#
# Description:
#   Reports whether fish is installed through the mechanism this unit uses, the
#   apt/dpkg package, by querying dpkg. The runner compares this against
#   unit::is_installed: present but unmanaged means fish arrived by some other route
...
#
# Returns:
#   0 when the fish package is installed
#   1 when the fish package is not installed
#
# Example:
#   unit::is_managed
#--------------------------------------------------
...
#
# Description:
#   fish has no inputs to collect. It only warms the sudo session, because the
...
#
# Description:
#   Installs fish, or updates it when already present, with apt-get. Each step is
...
unit::install() {
    ...
    output::run 'Installing the fish package' sudo apt-get install -y fish
}
...
#
# Description:
#   Purges fish with apt-get, announced through output::run so the user sees the
#   step and only sees apt-get's output if it fails. Uninstall is unconditional: it
#   removes fish regardless of whether this tool installed it, so a user who asks
...
#--------------------------------------------------
unit::uninstall() {
    output::run 'Purging the fish package' sudo apt-get purge -y fish
}
...
# ─── Imports ──────────────────────────────────────────────────────────────────
# shellcheck source=lib/software/fish/configure
source "$(dirname "${BASH_SOURCE[0]}")/../../../lib/software/fish/configure"
