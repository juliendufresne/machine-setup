# Specs for the tree unit on Ubuntu 26.04. TEST_FLAG keeps the unit's functions
...
Describe 'libexec/ubuntu_26.04/software/tree.sh'
    TEST_FLAG=true
    Include libexec/ubuntu_26.04/software/tree.sh

    # Isolated host and a log to spy on apt-get. TREE_ON_PATH drives is_installed
    # (does `command -v tree` resolve) and TREE_PRESENT drives is_managed (does
    # `dpkg -s tree` report the package), so a test can pretend tree is present by
    # any means, present via our package, or absent, without touching the host.
    setup() {
        helper::isolate
        export MACHINE_SETUP_UNIT_NAME=tree
    ...
    # `command -v tree`; every other call falls through to the real builtin.
    command() {
        if [ "$1" = -v ] && [ "$2" = tree ]
        then [ "${TREE_ON_PATH}" = 1 ]
    ...
    dpkg()    { [ "${TREE_PRESENT}" = 1 ]; }            # `dpkg -s tree`
    ...
    Describe 'unit::is_installed'
        ...
        It 'is true when tree resolves on the host'
            TREE_ON_PATH=1
        ...
        It 'is false when tree resolves nowhere'
            TREE_ON_PATH=0
            When call unit::is_installed
    ...
    Describe 'unit::is_managed'
        ...
        It 'is true when dpkg reports the tree package present'
            TREE_PRESENT=1
        ...
        It 'is false when dpkg reports the tree package absent'
            TREE_PRESENT=0
            When call unit::is_managed
            The status should be failure
            The stdout should be blank
    ...
    Describe 'unit::is_configured'
        ...
        It 'is always satisfied because tree needs no configuration'
    ...
    Describe 'unit::install'
        ...
        It 'updates the package lists then installs tree and reports each step'
            When call unit::install
            The status should be success
            The line 1 of stdout should equal '  ✓ Updating the package lists'
            The line 2 of stdout should equal '  ✓ Installing the tree package'
            The stderr should be blank
...
install -y tree'
    ...
    Describe 'runner::status'
        ...
        It 'reports available when tree is not present'
            TREE_ON_PATH=0
        ...
        It 'reports unmanaged when tree is present but not via our package'
            TREE_ON_PATH=1
            TREE_PRESENT=0
            When call runner::status
            The status should be success
        ...
        It 'reports configured as soon as tree is present via our package'
            TREE_ON_PATH=1
            TREE_PRESENT=1
            When call runner::status
            The status should be success
    ...
    Describe 'unit::configure'
        ...
        It 'does nothing because tree needs no configuration'
    ...
    Describe 'unit::unconfigure'
        ...
        It 'does nothing because tree has no configuration to restore'
    ...
    Describe 'unit::uninstall'
        ...
        It 'purges tree and reports the step'
            When call unit::uninstall
            The status should be success
            The stdout should equal '  ✓ Purging the tree package'
            The stderr should be blank
            The contents of file "$APT_LOG" should equal 'purge -y tree'
        End

        It 'purges tree even when this tool never recorded installing it'
